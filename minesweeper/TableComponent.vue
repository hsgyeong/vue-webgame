...
import {CODE, NORMALIZE_CELL, OPEN_CELL, FLAG_CELL, QUESTION_CELL, CLICK_MINE} from './store';
...
export default {
  computed: {
    ...
    cellDataStyle () {
      return (row, cell) => {
      switch (this.$store.state.tableData[row][cell]) {        // this를 사용할때 computed부분은 일반함수로, 그 내부는 화살표 함수로
        case CODE.NORMAL:
        case CODE.MINE:
          return {
            background: '#444',
